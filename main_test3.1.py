...
# テキスト分割関数 (LangChain RecursiveCharacterTextSplitter)
def split_text(text):
...
## サイドバーの設定 ##
st.set_page_config(page_title="Local LLM Chat (Recursive)")
...
if st.sidebar.button("インデックス作成"):
    ...
    st.sidebar.success(f"インデックス作成完了 (チャンク数: {len(chunks)} / {file.name})")

# タイトル
st.title("Local LLM Chat (Recursive Split)")
